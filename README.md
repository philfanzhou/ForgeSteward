...
The current source targets **0.2.6 (unreleased)**, with five independent plugins: `check-workflow`, `prepare-work`, `execute-work`, `review-and-merge`, and `fix-feedback`. A source version is not proof of publication: confirm the matching tag and non-draft [GitHub Release](https://github.com/philfanzhou/ForgeSteward/releases). The latest published snapshot is described in the [v0.2.5 version matrix, changes and installation guide](docs/releases/v0.2.5.md); the unreleased 0.2.6 target makes check-workflow remove workflow additions written by its earlier versions.
...
| Skill | Purpose |
| --- | --- |
| [`forge-steward-check-workflow`](plugins/check-workflow/skills/forge-steward-check-workflow/SKILL.md) | Sync the standard ForgeSteward workflow block and agent entry import from a bundled template, then submit a PR or MR without merging. |
...
Explicit check-only requests remain read-only, and local-only/no-publish requests stop at that boundary. Implicit skill selection does not authorize publishing beyond the actual request. Repository permissions and approval gates still apply. It never changes business code, tests, CI or permissions, never merges, and never installs other skills. Supplementary rules written by earlier versions (0.2.4 and before) carry no markers. From the unreleased 0.2.6 target, the skill removes those it can confirm from both their content and git history as earlier check-workflow additions, in the same PR or MR as the block sync, and lists uncertain candidates for maintainers instead of deleting them. Rules the project wrote itself are never removed. Whenever a run modifies project files, the skill reports a per-file summary of the operation, the changes, line counts and the delivery status; otherwise it states that no project files were modified. Agent-specific loading details and limits are documented in the bundled [entrypoint reference](plugins/check-workflow/skills/forge-steward-check-workflow/references/agent-entrypoints.md).
