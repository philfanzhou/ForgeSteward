...
The current source targets **0.2.5**, with five independent plugins: `check-workflow`, `prepare-work`, `execute-work`, `review-and-merge`, and `fix-feedback`. A source version is not proof of publication: confirm the matching tag and non-draft [GitHub Release](https://github.com/philfanzhou/ForgeSteward/releases). See the [v0.2.5 version matrix, changes and installation guide](docs/releases/v0.2.5.md). This release replaces check-workflow's per-repository gap filling with a standard template block, and review-and-merge now merges changes that pass review without asking for confirmation.
