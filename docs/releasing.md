...
仓库与所有插件采用统一版本：`VERSION` 为 `X.Y.Z` 时，全部插件的包版本均为 `X.Y.Z`，发布 Tag 必须为 `vX.Y.Z`；候选版后缀也须完全一致。该规则从本次未发布的 `0.2.2` 开始执行，历史发布记录不改写。仓库级安装器随仓库 Tag 固定，目前没有独立安装器包版本。独立安装不等于独立升版，用户仍可仅安装其中一个插件；本项目未提供任意混搭各插件历史版本的解析器。
