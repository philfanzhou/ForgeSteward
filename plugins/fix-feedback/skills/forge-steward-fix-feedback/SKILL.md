...
4. Change Request 进入第三轮 Review（一次完整审查并给出结论计为一轮）时，先暂停写代码，逐 commit 审计范围和语义闭合；不能追溯到原验收的改动从本分支移出，另行登记为后续 Issue。第三轮之后仍反复出现跨文档、跨状态或跨数据流矛盾时，停止逐条增量修补，在原 Issue 和 Change Request 记录矛盾，按授权撤销原 Issue 的 ready 状态，交回准备阶段拆分或重建语义模型，该项以阻塞状态结束本轮；每个 commit 都能追溯到验收，不构成继续修补的理由。
...
- 存在无法继续的具体阻塞（包括第三轮后仍反复矛盾而交回准备阶段），已经记录事实、尝试过的解决方式和可执行下一步；或
