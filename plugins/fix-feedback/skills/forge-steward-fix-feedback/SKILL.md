...
4. 已完成 2 轮往返（定义见「冻结待修清单」第 5 步）的 Change Request，本次修复会构成第 3 轮：先暂停写代码，逐 commit 审计范围和语义闭合；不能追溯到原验收的改动从本分支移出，另行登记为后续 Issue，再修复必修项。推送后往返达到 3 轮，后续审查和修复均跳过，等待人工处理。
...
新发现的问题先检索去重并按仓库规范分诊。当前任务验收遗漏在原任务闭环，已关闭任务的遗漏回链原任务、Change Request 及验收条目并记为返工；独立既有缺陷登记或提出建议，同类缺陷归并到已有或同一个 Issue 追加证据，只有已确认可复现、独立且无同类登记的缺陷才新建；可选改进只在原 Issue 或 Change Request 记录，不新建；已接受的非保证没有新证据或范围变化时不反复登记为 bug。严重风险记录触发条件、实际影响、处置及恢复主线的条件；其他发现不自动扩充冻结清单。完成当前项后按下述格式交接，再安全清理由本轮创建且确认无未提交改动或未保存独有提交的临时环境，继续下一项。
